...
OUTPUT_DIR = Path("downsampled_psd")
SAMPLING_RATE = 128
MAX_FREQUENCY = 64
...
def plot_psd(frequencies, mean_psd, output_path, subject_label, max_frequency=MAX_FREQUENCY):
    ...
    ax.semilogy(frequencies[mask], mean_psd[mask], color="#9333ea", linewidth=2)
    ax.set_title(f"{subject_label} Downsampled Average Power Spectral Density")
